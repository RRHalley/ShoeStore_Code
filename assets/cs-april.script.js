var touch = false, clickEv = 'click', checkDropdown = [], checkSideMenu = [], checks_scroll = 0, fullPageActiveDesk = 0, fullPageActiveTab = 0;
...
function fullPage(){
  var activeFullPage = function(){
    $('#MainContent').fullpage({
      verticalCentered: true
    });
    fullPageActiveDesk = 1;
    fullPageActiveTab = 0;
  }
  var inActiveFullPage = function(){
  	$.fn.fullpage.destroy('all');
    fullPageActiveDesk = 0;
    fullPageActiveTab = 1;
  }
  var addCountItem = function(){
    var length = $('.index-section-banner').length;
    length = (length < 10) ? ('0'+length) : length;
    $('.index-section-banner').each(function(index) {
      var index = index + 1;
      index = (index < 10) ? ('0'+index) : index;
      $(this).find('.current').html(index);
      $(this).find('.size').html('/'+length);
    });
  }
  addCountItem();
  if(getWidthBrowser() > 991 && fullPageActiveDesk == 0){
	activeFullPage();
  }
  else if(getWidthBrowser() < 992 && fullPageActiveTab == 0 && fullPageActiveDesk == 1){
	inActiveFullPage();
  }
}
